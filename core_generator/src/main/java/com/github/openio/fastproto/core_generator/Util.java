...
public class Util {
    ...
    static File genFile(String dir, String pack, String fileName) throws IOException {
        ...
        if (!fileDir.isDirectory()) {
            if (!fileDir.mkdirs()) {
                recordLoggerOne(ErrorMessage.ERROR_GEN_FILE.getMethodName(), new String[]{dir, pack, fileName},ErrorMessage.ERROR_GEN_FILE.getMessage()+fileDir.toPath());
                throw new RuntimeException("can not mkdirs:" + fileDir.toPath());
            }
        }
        ...
        if (!file.exists()) {
            if (!file.createNewFile()) {
                recordLoggerOne(ErrorMessage.ERROR_GEN_FILE.getMethodName(), new String[]{dir, pack, fileName},ErrorMessage.ERROR_GEN_FILE.getMessage()+file.toPath());
                throw new RuntimeException("can not creatFile:" + file.toPath());
            }
        }
        return file;
    ...
    private static void recordLoggerOne(String errorMethod, String[] inputParams, String message) {
        if(inputParams.length > 1) {
            StringBuilder builder = new StringBuilder(inputParams[0]);
            for (int i = 1; i < inputParams.length; i++) {
                builder.append("、").append(inputParams[i]);
            }
            logger.error("errorMethod:{} , inputParams:{} and errorMessage:can not mkdirs:{}",errorMethod, builder.toString(), message);
        }
        if(inputParams.length == 1) {
            logger.error("errorMethod:{} , inputParams:{} and errorMessage:can not mkdirs:{}",errorMethod, inputParams[0], message);
